...
class AdvancedEDA:
    ...
    def get_column_metadata(self) -> list[dict]:
        ...
        for col in self.df.columns:
            ...
            iqr_out = 0  # Default for non-numeric
            ...
            # Assign quality score once at the end
    ...
    # -------------------------------------------------------------------------
    #  Numerical Summary (Refactored to use cached metadata)
    # -------------------------------------------------------------------------

    def get_numerical_summary(self, metadata: list[dict] = None) -> dict:
        """Extended descriptive statistics extracted efficiently from cached metadata."""
        num_df = self.df.select_dtypes(include="number")
        if num_df.empty:
            return {"available": False, "reason": "No numeric columns."}

        meta = metadata if metadata is not None else self.get_column_metadata()
        out: dict[str, Any] = {}
        
        for col_data in meta:
            if "numeric" in col_data:
                c = col_data["feature"]
                num = col_data["numeric"]
                out[c] = _jsonable({
                    "count":          len(self.df) - col_data["missing_count"],
                    "mean":           num.get("mean"),
                    "std":            num.get("std_dev"),
                    "min":            num.get("min"),
                    "p25":            num.get("p25"),
                    ...
                    "p75":            num.get("p75"),
                    "max":            num.get("max"),
                    "skewness":       num.get("skewness"),
                    "kurtosis":       num.get("kurtosis"),
                    "iqr_outliers":   num.get("outliers_iqr"),
                    "zscore_outliers": num.get("outliers_zscore"),
                })
        return out
    ...
    # -------------------------------------------------------------------------
    #  Datetime / Time-Series Diagnostics
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    #  Outlier Summary (Refactored to use cached metadata)
    # -------------------------------------------------------------------------

    def get_outlier_summary(self, metadata: list[dict] = None) -> dict:
        """IQR and Z-score outlier report extracted efficiently from cached metadata."""
        num_df = self.df.select_dtypes(include="number")
        if num_df.empty:
            return {"available": False, "reason": "No numeric columns."}
        ...
        for col_data in meta:
            if "numeric" in col_data:
                num = col_data["numeric"]
                iqr_out = num.get("outliers_iqr", 0)
                records.append(_jsonable({
                    "feature":           col_data["feature"],
                    "iqr_outliers":      iqr_out,
                    "iqr_outlier_pct":   num.get("outliers_iqr_pct"),
                    "zscore_outliers":   num.get("outliers_zscore"),
                    "zscore_outlier_pct": num.get("outliers_zscore_pct"),
                    "lower_fence":       num.get("lower_fence_iqr"),
                    "upper_fence":       num.get("upper_fence_iqr"),
    ...
    # -------------------------------------------------------------------------
    #  Data Quality Scorecard (Refactored to use cached metadata)
    # -------------------------------------------------------------------------

    def get_data_quality_scorecard(self, metadata: list[dict] = None) -> dict:
    ...
    # -------------------------------------------------------------------------
    #  Full Report (Refactored to implement data caching)
    # -------------------------------------------------------------------------

    def run_full_report(self, corr_method: str = "pearson") -> dict[str, Any]:
        """
        Execute all analysis sections efficiently. Passes calculated metadata downstream 
        to prevent redundant calculations.
        """
        # Execute the heavy metadata parsing ONCE
        meta = self.get_column_metadata()
        
        self._report = {
            "dataset_overview":     self.get_dataset_overview(),
            "column_metadata":      meta,
            # Pass cached metadata downstream to save redundant compute loops
            "numerical_summary":    self.get_numerical_summary(metadata=meta),
            ...
            "outlier_summary":      self.get_outlier_summary(metadata=meta),
            "missing_value_report": self.get_missing_value_report(),
            "quality_scorecard":    self.get_data_quality_scorecard(metadata=meta),
        }
        return self._report
...
if __name__ == "__main__":
    ...
    ts_messy = base_ts.to_series().sample(frac=1, random_state=0)
    ts_messy.iloc[10] = pd.Timestamp("1970-01-01")
    ts_messy.iloc[20] = pd.Timestamp("9999-12-31")
